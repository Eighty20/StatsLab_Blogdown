...
```{r}
data_characters_request <- httr::GET(url = "https://anapioficeandfire.com/api/characters?pageSize=1000")
...
GOT_df <-
  tibble::tibble(char_name = data_characters %>% map_chr("name"),
                 data_char = data_characters) %>% 
    mutate(gender = data_char %>% map_chr("gender")) %>% 
    mutate(culture = data_char %>% map_chr("culture")) %>% 
    mutate(born = data_char %>% map_chr("born")) %>% 
    mutate(died = data_char %>% map_chr("died")) %>% 
    mutate(titles = data_char %>% map("titles")) %>% 
    mutate(aliases = data_char %>% map("aliases")) %>% 
    mutate(father = data_char %>% map_chr("father")) %>% 
    mutate(mother = data_char %>% map_chr("mother")) %>% 
    # mutate(spouse = data_char %>% map(~.x %>% pluck("spouse") %>% httr::GET())) %>% 
    # mutate(allegiances = data_char %>% map(~.x %>% pluck("allegiances") %>% httr::GET())) %>%
    mutate(playedBy = data_char %>% map("playedBy"))
